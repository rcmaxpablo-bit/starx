...
module.exports = (client) => {
  ...
  async function publishCustomerPanel(reason = 'system') {
    ...
    customerPanelPromise = (async () => {
      const channel = await getTextChannel(CUSTOMER_PANEL_CHANNEL_ID);
      ...
      console.log(
        `✅ PANEL KLIENTA (${reason}): wiadomość=${message.id}, kanał=${CUSTOMER_PANEL_CHANNEL_ID}`
      );
  ...
  async function responseFor(interaction) {
    ...
    if (choice === 'history') {
      const txs = store.read('transactions')
        .filter(t => t.userId === interaction.user.id && t.status === 'confirmed')
  ...
  client.on(Events.MessageCreate, async message => {
    ...
    scheduleRename();
    publishCustomerPanel(`+rep ${message.id}`).catch(err =>
      console.error('❌ PANEL KLIENTA PO +REP:', err?.stack || err)
    );
