...
function recordTransaction({
  ...
  // Samo przygotowanie legit checka nie jest jeszcze transakcją klienta.
  // Statystyki zostaną zwiększone dopiero po zapisaniu faktycznej wiadomości +rep.
  return {
    transaction,
    created: true
  };
...
function rebuildCustomersFromTransactions() {
  ...
  for (const transaction of transactions) {
    if (!transaction?.userId || transaction.status !== 'confirmed') {
      continue;
    }
...
function setPanelMessageId(panelKey, messageId) {
  ...
  settings.panelMessages =
    settings.panelMessages && typeof settings.panelMessages === 'object' && !Array.isArray(settings.panelMessages)
      ? settings.panelMessages
      : {};
  settings.panelMessages[key] = id;

  write('settings', settings);
  return id;
}

function deletePanelMessageId(panelKey) {
  const key = String(panelKey || '').trim();
  if (!key) return false;

  const settings = read('settings');
  if (!settings.panelMessages || !Object.prototype.hasOwnProperty.call(settings.panelMessages, key)) {
    return false;
  }
